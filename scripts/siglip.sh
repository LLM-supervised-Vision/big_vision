...
python -m big_vision.trainers.proj.image_text.siglip \
    --config big_vision/configs/proj/image_text/siglip_replication.py:loss_fn='softmax',batch_size=8192,scan=True,fsdp=2,dtype='bfloat16',debug=False \
    --workdir gs://us-central2-storage/tensorflow_datasets/clip_replication_bs8192_`date '+%m-%d_%H%M'`
