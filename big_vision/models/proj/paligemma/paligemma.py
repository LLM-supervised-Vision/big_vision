...
class Model(nn.Module):
  ...
  def embed_image_and_text(self, image, text, *,
    ...
    if zimg is not None and not is_blind:
  ...
  def __call__(self, image, text, mask_ar, encoded=None, is_blind=False, train=False):
    ...
    text_pre_logits = out["llm/pre_logits"][:, out["img/zimg"].shape[1]:, :] if "img/zimg" in out and not is_blind else out["llm/pre_logits"]
