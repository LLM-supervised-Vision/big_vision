...
flags.DEFINE_boolean("debug", False, "Debug mode: whether to perform upload_from_string().")

file_operation_lock = threading.Lock()


def get_blobs_sorted(destination_dir):
    # Get the bucket
    ...
    blobs = list(bucket.list_blobs(prefix=prefix[:-2])) # tensorflow_datasets/laion400m/images/1.0
    ...
    logging.info(f"{FLAGS.worker_id}: len(blobs): {len(blobs)}")

    # Sort blobs based on the custom sorting key
    blobs_sorted = sorted(blobs, key=get_version)
    logging.info(f"{FLAGS.worker_id}: len(blobs_sorted): {len(blobs_sorted)}")

    return bucket, prefix, blobs_sorted


def update_dataset_info_json(bucket):
    ...
    for blob in blobs_sorted:
        if blob.name.split("/")[-1] == "dataset_info.json":
            ...
            # check num of files in the version folder that starts with file_prefix
            ...
            num_files = len([blob for blob in blobs_in_folder if blob.name.split("/")[-1].startswith(file_prefix)])
    ...
    if first_folder_blob and worker_id==0:
        ...
        if not FLAGS.debug: first_folder_blob.upload_from_string(json.dumps(dataset_info))
    ...
    blobs_sorted = [blob for blob in blobs_sorted if blob.name.split("/")[-1].startswith(file_prefix)]
...
def move_and_rename_files():
    ...
    destination_dir = f"{FLAGS.tfds_data_dir}/{FLAGS.tfds_name}/1.0.0" # gs://us-central2-storage/tensorflow_datasets/laion400m/images/1.0.0/

    bucket, prefix, blobs_sorted = get_blobs_sorted(destination_dir)
    ...
    for i, blob in enumerate(blobs_sorted):
        ...
        if new_file_index%num_workers==worker_id:
            ...
            new_file_name = f"{file_prefix}-{new_file_index:05d}-of-{total_num_files:05d}"
    ...
    blobs = [blob for blob in blobs if blob.name.split("/")[-1].startswith(file_prefix)]
...
def main(argv):
    logging.info(f"worker {FLAGS.worker_id}: args.tfds_name: {FLAGS.tfds_name}")
    logging.info(f"worker {FLAGS.worker_id}: args.split: {FLAGS.split}")
    logging.info(f"worker {FLAGS.worker_id}: args.tfds_data_dir: {FLAGS.tfds_data_dir}")
    logging.info(f"worker {FLAGS.worker_id}: args.final_version_id: {FLAGS.final_version_id}")
    logging.info(f"worker {FLAGS.worker_id}: args.num_workers: {FLAGS.num_workers}")
    logging.info(f"worker {FLAGS.worker_id}: args.worker_id: {FLAGS.worker_id}")
    logging.info(f"worker {FLAGS.worker_id}: args.debug: {FLAGS.debug}")
