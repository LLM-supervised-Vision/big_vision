...
r"""Trains a CapPa model (https://arxiv.org/abs/2306.07915) on cc12m.
...
def get_config(arg=None):
  ...
  pp_cc12m = (f'decode|{pp_image}|'
              'choice(inkey="caption", outkey="text")|'
              f'{tokenizer("text", "labels")}|keep("image", "labels")')
  config.input.pp = pp_cc12m
  config.input.data = dict(name='cc12m', split='train')
  ...
  config.model_init = {'encoder': 'gs://us-central2-storage/tensorflow_datasets/cappa_bs16384_warm0.02_lr1e-3_wd1e-4_bf16_b2-0.95_6lyr_06-15_2102/checkpoint.bv-000183105:encoder'}
  # also exclude  and Transformer/encoderblock/MultiHeadDotProductAttention_0/query_ln/scale
  config.model_load = {'dont_load': ['head/kernel', 'head/bias', 'Transformer/encoderblock/MultiHeadDotProductAttention_0/key_ln/scale', 'Transformer/encoderblock/MultiHeadDotProductAttention_0/query_ln/scale']}
  ...
  schedule = [
    ('img/.*', None),
    ('.*', dict(decay_type='cosine',warmup_steps=max(int(0.02 * config.total_steps), 100) if not config.runlocal else 5))
  ]
