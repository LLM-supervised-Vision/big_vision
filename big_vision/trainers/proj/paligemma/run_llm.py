...
def load_model(ckpt):
  model_cfg = ml_collections.FrozenConfigDict(dict(
      ...
      # llm=dict(lyrs_frozen=9,vocab_size=256_000 + 1024 + 128,scan=True,dropout=0.0,pool='gap'),
  ))
...
def main(argv):
  info(f"{argv=}")
  info("Loading model...")
  model, params = load_model(CKPT.value) if not 'gemma2b' in CKPT.value else load_model({'img': None, 'llm': CKPT.value})
  ...
  decode = functools.partial(
      ...
      sampler=SAMPLER.value,replicate_out=True)

  def make_batch(fname, prompt):
    image = open(fname, "rb").read() if fname is not None else None
